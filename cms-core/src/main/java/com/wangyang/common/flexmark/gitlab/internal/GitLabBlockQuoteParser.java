...
//
            ...
            if (options.nestedBlockQuotes || !haveBlockQuoteParser(state) || lastChild instanceof GitLabBlockQuote) {
                ...
                if (matcher.matches()) {
                    GitLabBlockQuoteParser gitLabBlockQuoteParser = new GitLabBlockQuoteParser(state.getProperties(), line.subSequence(0, 3), line.subSequence(matcher.start(1), matcher.end(1)));
